...
- **Zdroje pravdy:**
  ...
  - `Kosik.dc.html` — box košíku v hlavičce a **jediné místo výběru jazyka**. Box drží cenu (tlačítko do košíku), za vertikálním oddělovačem `#D5DAE0` je přepínač jazyka: jen vlajka 25 × 16 (rádius 4) + chevron 18 px, který otevírá seznam `CZ` / `SK` s vlajkami. Jazyk určuje měnu — `CZ` = CZK, `SK` = EUR; komponenta hlásí dál měnu (`onCurrencyChange('CZK'|'EUR')`) a aktivní jazyk si z ní odvozuje. Vlajky jsou samostatné assety `vapp-flag-cz.svg` / `vapp-flag-sk.svg`, ne ikony v `Ikony.dc.html` (barevné, mimo kontrakt 24 × 24 / currentColor). Ikona měny (`coin`) v akční skupině hlavičky je zrušená. Bezstavový vůči stránce: `currency`, `currencies` jsou data, změna se hlásí `onCurrencyChange(code)`; otevřeno/zavřeno drží komponenta (Escape a klik mimo zavírají).
